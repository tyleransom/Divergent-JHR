clear
clc
delete('Logs/divergent_counterfactuals_no_RtoR.diary');
diary('Logs/divergent_counterfactuals_no_RtoR.diary');
diary on
...
b=fminsearch('counter2',[0.0;0.0],options)
feval('counter2',b)

% Using the admissions adjustments, calculate admits and shares
disp('******Counterfactual 2 (R to R correction)******')
disp(['delta0 = ',num2str(b(1))])
disp(['deltaL = ',num2str(b(2))])
w_l_adm=exp(w_l_index+b(1)+b(2))/(1+exp(w_l_index+b(1)+b(2)))*w_l_app;
w_nl_adm=exp(w_nl_index+b(1))/(1+exp(w_nl_index+b(1)))*w_nl_app;
white_admits=round(w_l_adm+w_nl_adm)
white_LA_share=w_l_adm/(w_l_adm+w_nl_adm)
...
b=fminsearch('counter3',[0.0;0.0],options)
feval('counter3',b)

% Using the admissions adjustments, calculate admits and shares
disp('******Counterfactual 3 (R to R correction)******')
w_l_adm=exp(w_l_index+b(1)+b(2))/(1+exp(w_l_index+b(1)+b(2)))*w_l_app;
w_nl_adm=exp(w_nl_index+b(1))/(1+exp(w_nl_index+b(1)))*w_nl_app;
white_admits=round(w_l_adm+w_nl_adm)
white_LA_share=w_l_adm/(w_l_adm+w_nl_adm)
